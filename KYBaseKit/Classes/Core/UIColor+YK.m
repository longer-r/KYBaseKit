...
void KY_RGB2HSL(CGFloat r, CGFloat g, CGFloat b,
...
void KY_HSL2RGB(CGFloat h, CGFloat s, CGFloat l,
...
void KY_RGB2HSB(CGFloat r, CGFloat g, CGFloat b,
...
void KY_HSB2RGB(CGFloat h, CGFloat s, CGFloat v,
                 CGFloat *r, CGFloat *g, CGFloat *b) {
    CLAMP_COLOR_VALUE(h);
    CLAMP_COLOR_VALUE(s);
    CLAMP_COLOR_VALUE(v);
...
void KY_RGB2CMYK(CGFloat r, CGFloat g, CGFloat b,
...
void KY_CMYK2RGB(CGFloat c, CGFloat m, CGFloat y, CGFloat k,
                  CGFloat *r, CGFloat *g, CGFloat *b) {
    CLAMP_COLOR_VALUE(c);
    CLAMP_COLOR_VALUE(m);
    CLAMP_COLOR_VALUE(y);
...
void KY_HSB2HSL(CGFloat h, CGFloat s, CGFloat b,
...
void KY_HSL2HSB(CGFloat h, CGFloat s, CGFloat l,
...
+ (UIColor *)ky_colorWithHue:(CGFloat)hue
                   saturation:(CGFloat)saturation
                    lightness:(CGFloat)lightness
                        alpha:(CGFloat)alpha {
    CGFloat r, g, b;
    KY_HSL2RGB(hue, saturation, lightness, &r, &g, &b);
...
+ (UIColor *)ky_colorWithCyan:(CGFloat)cyan
                       magenta:(CGFloat)magenta
                        yellow:(CGFloat)yellow
                         ...
                         alpha:(CGFloat)alpha {
    CGFloat r, g, b;
    KY_CMYK2RGB(cyan, magenta, yellow, black, &r, &g, &b);
    return [UIColor colorWithRed:r green:g blue:b alpha:alpha];
}
...
static inline NSUInteger ky_hexStrToInt(NSString *str) {
    uint32_t result = 0;
    sscanf([str UTF8String], "%X", &result);
    return result;
}

static BOOL ky_hexStrToRGBA(NSString *str,
                             CGFloat *r, CGFloat *g, CGFloat *b, CGFloat *a) {
    str = [[str ky_stringByTrim] uppercaseString];
    if ([str hasPrefix:@"#"]) {
    ...
    //RGB,RGBA,RRGGBB,RRGGBBAA
    if (length < 5) {
        *r = ky_hexStrToInt([str substringWithRange:NSMakeRange(0, 1)]) / 255.0f;
        *g = ky_hexStrToInt([str substringWithRange:NSMakeRange(1, 1)]) / 255.0f;
        *b = ky_hexStrToInt([str substringWithRange:NSMakeRange(2, 1)]) / 255.0f;
        if (length == 4)  *a = ky_hexStrToInt([str substringWithRange:NSMakeRange(3, 1)]) / 255.0f;
        else *a = 1;
    } else {
        *r = ky_hexStrToInt([str substringWithRange:NSMakeRange(0, 2)]) / 255.0f;
        *g = ky_hexStrToInt([str substringWithRange:NSMakeRange(2, 2)]) / 255.0f;
        *b = ky_hexStrToInt([str substringWithRange:NSMakeRange(4, 2)]) / 255.0f;
        if (length == 8) *a = ky_hexStrToInt([str substringWithRange:NSMakeRange(6, 2)]) / 255.0f;
        else *a = 1;
    }
    return YES;
}

+ (instancetype)ky_colorWithHexString:(NSString *)hexStr {
    CGFloat r, g, b, a;
...
- (NSString *)ky_hexStringWithAlpha {
    return [self ky_hexStringWithAlpha:YES];
}

- (NSString *)ky_hexStringWithAlpha:(BOOL)withAlpha {
    ...
    if (hex && withAlpha) {
        hex = [hex stringByAppendingFormat:@"%02lx",
               (unsigned long)(self.ky_alpha * 255.0 + 0.5)];
    }
    return hex;
}
...
- (UIColor *)ky_colorByChangeHue:(CGFloat)h saturation:(CGFloat)s brightness:(CGFloat)b alpha:(CGFloat)a {
    CGFloat hh, ss, bb, aa;
...
- (BOOL)ky_getHue:(CGFloat *)hue
    ...
    KY_RGB2HSL(r, g, b, hue, saturation, lightness);
    *alpha = a;
    return YES;
}
...
- (BOOL)ky_getCyan:(CGFloat *)cyan
    ...
    KY_RGB2CMYK(r, g, b, cyan, magenta, yellow, black);
    *alpha = a;
    return YES;
}
...
- (CGFloat)ky_saturation {
    CGFloat h, s = 0, b, a;
    [self getHue:&h saturation:&s brightness:&b alpha:&a];
    return s;
}

- (CGFloat)ky_brightness {
    CGFloat h, s, b = 0, a;
    [self getHue:&h saturation:&s brightness:&b alpha:&a];
    return b;
}

- (CGColorSpaceModel)ky_colorSpaceModel {
    return CGColorSpaceGetModel(CGColorGetColorSpace(self.CGColor));
}

- (NSString *)ky_colorSpaceString {
